...
Objections to MAS\\
\begin{description} 
	\item [Isn’t it all just Distributed/Concurrent Systems?]
	\item [Isn’t it all just]
	\item [Isn’t it all just Economics/Game Theory?]
	\item [Isn’t it all just Social Science?]
\end{description}
...
of the steps can add new targets to fulfill which will keep the agent exploring the environment and interacting
with it and other agents to develop its strategies to solve the tasks.
%%TODO posa el grafic del cicle d'un agent de la presentacio dels Flintstones.

An example of very simple agent would be a thermostat. It samples the environment with a probe/sensor, checks if the
temperature corresponds to the target. According to the measure it launches one of the two actions, to set
the heater to ON or to OFF.\\
...
\begin{description}
	...
	\begin{description}
		...
		For example: You have one TV in your house; you want to watch a movie, your housemate wants to
		watch football. A possible deal: watch football tonight, and a movie tomorrow.
...
\begin{description} Agent Architectures %% we are interested in this division
...
Differences between Agents & Expert Systems pg17
and who this fits in out models

will we use BDIs? responc aixo a la part d'agents dins l'ABM?
