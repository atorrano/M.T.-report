...
parts and the simple composition of those analisys. Usually, reductionism has coped with complicated systems
where the supression of constituents does not change the main trend of its behaviour, and that is used to
...
Complex science tries to find out how the interactions of a set of participants conform a system that exhibits 
adaptive traits, descentralized organization, and macroscale behaviour patterns. 
The strong relationships that give arise to the complex phenomena and acts as the glue that holds the system ...
it gives reason to the system existence, as any change encompass the dynamics of the system it also makes it
vulnerable...  adaptability + robustness + irreducibility

\begin{figure}[tp]
\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}
\fbox{\includegraphics[width=60mm,keepaspectratio=true]{figures/BZ_Spiral_waves.jpg}}
...
Indeed we observe that theres a system there is the pressence of the macro phenomena. It is the evidence that
something is there
...
%TODO exemples : swarm, ecosystems, Belousov-Zhabotinsky reaction
...
which large networks of components with no central control and simple rules of operation give rise to complex collective 
behavior, sophisticated information processing, and adaptation via learning or evolution. Sumarizing, a system that 
...
of a higher level pattern,phenomenon or global behaviour.
Emergence is one of the key concepts from Complexity that is studied in Social Simulation. Indeed,
emergence is one of the main features exhibited by complex systems.
...
considered the bottom of the process. The think is that due to nonlinear relationships...
...
arise from constituents interactions. It is here where Emergence can be appear. When a pattern is 
recognizable and repeats usually along experiments, is plausible to be considered emergent \cite{Holland.Emergence.1997}.  

Reductionism -> divide the system to constituents. From constituents studied behaviour plus additive composition
we get the picture of the system.
...
Exemple : Volterra, dinamica equilibrada, Column fire run away

Examples : ant colony, swarms, self-organization of structures (extend it)
