...
\end{equation}
An agent $X_i$ retrieves information from environment states to choose actions to launch
\begin{equation}
	X_i : \Omega \longrightarrow A 
\end{equation}
An agent's trace or run is a sequence of states of the environment where 
the transitions were triggered by actions launched by the agents
\begin{equation}
	r = ( \omega_0 , \alpha_0 , \omega_1 , \alpha_1 ,\ldots, \omega_u-1 , \alpha_u-1 , \omega_u )
\end{equation}
\begin{equation}
	\forall j \in [0..u-1] : X_i(\omega_j) = \alpha_j
\end{equation}
\\
\textbf{Agent Main Features}
\begin{description}
